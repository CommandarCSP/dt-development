...
- **이름이 아니라 작동 방식을 그린다.** `[캐시]` 박스 하나는 글보다 못하다. 무엇 사이에 앉아 있고 무엇이 지나가는지를 그린다.
- **화살표에 라벨을 단다.** 라벨 없는 화살표는 "무슨 관계 있음"이고, `IPC 로 요청` 은 정보다.
...
- **`viewBox` 로 크기를 정한다.** `width`·`height` 속성을 박지 않는다 — A4 폭을 넘으면 G5 가 막는다. 폭 640 안쪽이 안전하다.
...
강조 화살표(`hb-arrow-accent`)를 경계를 넘는 한 곳에만 썼다. 그게 이 그림의 주장이다.
